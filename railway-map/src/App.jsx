...
// --- Train Data ---
const initialTrains = [
  { id: '19670', x: 150, y: 500, currentRoute: 0, routes: [['T_W_UP_IN', 'P_W_UP_PL3', 'PL3', 'P_E_PL3_UP', 'T_E_UP_OUT']] },
  { id: '12165', x: 1450, y: 550, currentRoute: 0, routes: [['T_E_DOWN_IN', 'ML_DOWN', 'T_W_DOWN_OUT']] },
  { id: '22500', x: 150, y: 500, currentRoute: 0, routes: [['T_W_UP_IN', 'P_W_UP_PL4', 'PL4', 'P_E_PL4_UP', 'T_E_UP_OUT']] },
  { id: '12381', x: 150, y: 500, currentRoute: 0, routes: [['T_W_UP_IN', 'P_W_UP_PL3', 'PL3', 'P_E_PL3_UP', 'T_E_UP_OUT']] },
];
...
export default function App() { 
  ...
  const handleAIGuidedRoute = async () => {
    if (!selectedTrain) return;

    // Create a new requestData object with only the data the backend needs.
    const requestData = {
        train_id: selectedTrain.id,
        yard_data: {
            tracks: yardData.tracks,
            points: yardData.points,
            platforms: yardData.platforms,
            signals: yardData.signals,
            labels: yardData.labels
        }
    };

    try {
        const response = await fetch('http://127.0.0.1:8000/api/get-route', {
            method: 'POST',
        ...
        setTrains(currentTrains => 
            currentTrains.map(t => 
                t.id === suggestion.id 
                    ? { ...t, routes: [suggestion.route], currentRoute: 0 } 
                    : t
            )
        );
        ...
        <div className="panel-grid"> 
          ...
          {selectedTrain && ( 
            <div> 
              ...
              <button onClick={handleAIGuidedRoute} className="btn btn-action">
                Get AI-Guided Route
              </button>
            </div> 
          )} 
